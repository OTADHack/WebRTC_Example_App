...
var path         = require('path')
  ...
  , logger       = require(path.join(__dirname, '..', '..', 'logger')).child({component : 'trace_aggregator'})
  ;
...
TraceAggregator.prototype.isBetter = function isBetter(scope, duration, apdexT) {
   ...
   */
  var isOverThreshold;
  if (this.config.transaction_tracer &&
      this.config.transaction_tracer.trace_threshold &&
      this.config.transaction_tracer.trace_threshold !== 'apdex_f' &&
      typeof this.config.transaction_tracer.trace_threshold === 'number') {
    isOverThreshold = duration > this.config.transaction_tracer.trace_threshold * 1000;
  }
  else {
    isOverThreshold = duration > 4 * 1000 * apdexT;
  }
   ...
   *
   * TODO: Not sure I'm interpreting this all entirely correctly.
   */
   ...
   * Ensure this scope is either already in the request time map
   * or that the map still hasn't hit capacity.
   */
  if (hasMetGuarantee &&
      !this.requestTimes[scope] &&
